...
const tempCases = [
  ...
  {
    tag: "HEALTHCARE",
    title: "Huma, a remote patient monitoring platform",
    subtitle: "An application that advances connected care for patients and accelerates research and therapies.",
    imgDesktop: "/img/iphone.png",
    imgMobile: "/img/iphone-mobile.png",
    url: {
      title: "View Case Study",
      link: "/case-studies/huma"
    },
    aditionLink: {
      title: "huma.com",
      href: "https://www.huma.com/",
      target: '_blank'
    }
  },
  {
    tag: "HEALTHCARE",
    title: "Beat Metric, a heart rate measurement app",
    subtitle: "We designed and developed a precise, user-friendly heart rate monitoring tool, integrating seamlessly with health devices for fitness, medical, or wellness tracking.",
    imgDesktop: "/img/iphone-red.png",
    imgMobile: "/img/iphone-mobile.png",
    url: {
      title: "View Case Study",
      link: "/case-studies/beat-metric"
    }
  }
]
  ...
  return (
    <div className={css.cases}>
      ...
      {tempCases.map(data =>
        ...
        <div className={`${css.health}`} key={data.title}>
          <Row>
            <div className={css.wrapp}>
              <div className={css.image}>
                <MyImage src={data.imgDesktop} alt="Phone" width={381} height={772} imgClass={css.desk} />
                <MyImage src={data.imgMobile} alt="Phone" width={270} height={545} imgClass={css.mobile} />
              </div>
              <div className={css.content}>
                <div className={css.tag}>
                  <p>{data.tag}</p>
                  <Link href={data.url.link} className={css.mobile_link} title={data.url.title} ></Link>
                </div>
                <h2>{data.title} </h2>
                <p className={css.subtitle}>{data.subtitle} </p>
                <div className={css.buttons}>
                  <Button type={['fill']} link={data.url.link}>
                    <>
                      {data.url.title}
                  ...
                  {data.aditionLink && <a className={css.huma} {...data.aditionLink}>
                    {data.aditionLink.title}
                    <div className={css.ico}>
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="none">
